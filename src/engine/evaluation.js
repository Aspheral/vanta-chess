...
export function evaluateBreakdown(position,perspective=position.turn){const attacks=buildAttackPair(position),phase=endgamePhase(position);const material=materialScore(position,perspective),activity=pieceSquareActivity(position,perspective),mobility=mobilityScore(position,perspective),pawns=pawnStructure(position,perspective,attacks),development=developmentScore(position,perspective),structure=rookAndBishopStructure(position,perspective),loose=loosePieceScore(position,perspective,attacks),ownKing=kingSafetyFor(position,perspective,attacks),enemyKing=kingSafetyFor(position,opposite(perspective),attacks),kingSafety=(ownKing-enemyKing)*1.5,kingDiscipline=(openingKingDiscipline(position,perspective)-openingKingDiscipline(position,opposite(perspective)))*1.35,attack=(attackPotential(position,perspective,attacks)-attackPotential(position,opposite(perspective),attacks))*1.15,initiative=tempoAndInitiative(position,perspective,attacks),endgame=endgameEvaluation(position,perspective),total=material+activity+mobility+pawns+development+structure+loose+kingSafety+kingDiscipline+attack+initiative+endgame;return{material,activity,mobility,pawns,development,structure,loose,kingSafety,kingDiscipline,attack,initiative,endgame,endgameWeight:phase.weight,total:Math.round(total)};}
