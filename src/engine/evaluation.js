...
import { attackReadiness, coordinatedAssaultValue } from './attack-plan.js';
    ...
    for (const sq of pawns) {
      ...
      if (passed) {
        ...
        if (progress >= 5) {
          ...
          if (!position.board[promoSq] && attacks[color].counts[promoSq] >= attacks[enemy].counts[promoSq]) bonus += 55;
        }
...
function rookAndBishopStructure(position, perspective) {
  let total = 0;
  for (const color of [WHITE, BLACK]) {
    const sign = signed(color, perspective), bishop = color === WHITE ? 'B' : 'b', rook = color === WHITE ? 'R' : 'r', pawn = color === WHITE ? 'P' : 'p', enemyPawn = color === WHITE ? 'p' : 'P';
    let bishops = 0;
...
function loosePieceScore(position, perspective, attacks) {
  ...
  for (let sq = 0; sq < 64; sq++) {
    ...
    const defended = Boolean(attacks[color].counts[sq]), value = PIECE_VALUES[typeOf(piece)] || 0;
    const exposure = defended ? (typeOf(piece) === 'q' ? 18 : typeOf(piece) === 'r' ? 12 : 7) : Math.max(10, Math.min(220, Math.round(value * .24)));
...
export function evaluateBreakdown(position, perspective=position.turn) {
  ...
  const coordination=(coordinatedAssaultValue(position,perspective)-coordinatedAssaultValue(position,opposite(perspective)))*0.65;
  const total=material+activity+mobility+pawns+development+structure+loose+kingSafety+kingDiscipline+attack+initiative+coordination;
  return {material,activity,mobility,pawns,development,structure,loose,kingSafety,kingDiscipline,attack,initiative,coordination,total:Math.round(total)};
}
...
export function personalityMoveBonus(position,move) {
  ...
  const aggressionScale=readinessBefore.assaultMultiplier;
  let bonus=0;
  ...
  if(givesCheck) bonus += Math.round(36*aggressionScale);
  ...
  bonus += Math.max(-15,Math.min(52,(afterAttack-beforeAttack)*.8*aggressionScale));
  const enemyKingBefore=kingSafetyFor(position,them,beforeAttacks), enemyKingAfter=kingSafetyFor(next,them,afterAttacks);
  bonus += Math.max(-10,Math.min(58,(enemyKingBefore-enemyKingAfter)*.55*aggressionScale));
  ...
  if(readinessBefore.phase!=='assault'&&readinessGain>0) bonus+=Math.min(30,Math.round(readinessGain*.95));

  if(position.fullmove<=12) {
    const type=typeOf(move.piece), undeveloped=undevelopedMinorCount(position,us);
    const forcingMaterial=Boolean(move.flags&FLAGS.CAPTURE)||Boolean(move.promotion);
    if(['n','b'].includes(type)) {
      if(isMinorHomeSquare(us,type,move.from)) bonus+=readinessBefore.phase==='mobilize'?18:10;
      else if(undeveloped>=2&&!forcingMaterial) bonus-=readinessBefore.phase==='mobilize'?26:18;
    }
    if(type==='p'&&[0,1,6,7].includes(move.from%8)&&undeveloped>=2&&!forcingMaterial) bonus-=12;
    if(type==='q'&&readinessBefore.developedMinors<3&&!forcingMaterial) bonus-=12;
  }

  if(readinessBefore.phase==='assault') {
    const joined=Math.max(0,readinessAfter.attackers-readinessBefore.attackers);
    bonus+=joined*10;
    if(givesCheck&&readinessAfter.attackers>=3) bonus+=10;
    if(afterAttack>beforeAttack) bonus+=Math.min(18,Math.round((afterAttack-beforeAttack)*.22));
  ...
  if(sacrifice>0) {
    const compensation=Math.max(0,afterAttack-beforeAttack)+(givesCheck?34:0)+Math.max(0,enemyKingBefore-enemyKingAfter)*.45;
    const unsupported=Math.max(0,sacrifice-compensation*3.5);
    const coordinatedComp=Math.min(1.3,Math.max(.65,aggressionScale));
    bonus+=Math.min(58,compensation*.72*coordinatedComp)-unsupported*.18;
  }
